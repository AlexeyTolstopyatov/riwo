...
func init() {
	AppRegistry["Player"] = AppPlayer
}

func AppPlayer(window *wm2.Window) {
	document := js.Global().Get("document")
	currentTheme := "blue"
	colorFG := wm2.GetColor[currentTheme]["vivid"]
	colorBG := wm2.GetColor[currentTheme]["faded"]
	colorMG := wm2.GetColor[currentTheme]["normal"]
	...
	container.Get("style").Set("backgroundColor", wm2.GetColor[currentTheme]["faded"])
	...
	timeDisplay.Get("style").Set("color", wm2.GetColor[currentTheme]["vivid"])
	...
	statusText.Get("style").Set("color", wm2.GetColor[currentTheme]["vivid"])
